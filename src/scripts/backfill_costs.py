...
async def backfill():
            ...
            for key in keys:
                ...
                try:
                    ...
                    if "o1" in model.lower():
                        logger.info(f"FOUND O1-like key: {key}, Model: {model}, Cost: {data.get('cost_estimate')}")
                    ...
                    if model == "":
                         ...
                         if tokens > 500 and cost < 0.002:
                             logger.info(f"SUSPICIOUS EMPTY MODEL: Key={key}, Tokens={tokens}, Cost={cost}. Likely mislabeled.")
                    ...
                    if "o1" in model.lower() and "mini" not in model.lower():
                        ...
                        correct_cost = (input_tokens * O1_INPUT_COST_PER_TOKEN) + (output_tokens * O1_OUTPUT_COST_PER_TOKEN)
                        ...
                        if abs(correct_cost - current_cost) > 0.001:
                            logger.info(f"Fixing {key} | Model: {model} | Tokens: {input_tokens}/{output_tokens} | Cost: {current_cost:.5f} -> {correct_cost:.5f}")
    ...
    logger.info(f"Backfill complete. Scanned: {scanned_count} keys, Updated: {updated_count} records")
