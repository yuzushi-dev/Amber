...
async def main():
    ...
    async with session_maker() as session:
        ...
        stmt = select(Document).options(selectinload(Document.chunks)).where(Document.filename.ilike("%Backup%"))
        ...
        if not doc:
            print(f"The specific file '{filename}' was not found in the list (this shouldn't happen if it was found before).")
            return
