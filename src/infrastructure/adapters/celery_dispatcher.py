...
class CeleryTaskDispatcher(TaskDispatcher):
    ...
    async def dispatch(self, task_name: str, args: list[Any] | None = None, kwargs: dict[str, Any] | None = None) -> str:
        args = args or []
        kwargs = kwargs or {}
        ...
        try:
            ...
            if celery_app.conf.task_always_eager:
                if task_name in celery_app.tasks:
                    ...
                    logger.warning(f"Task {task_name} not found in registry for eager execution. Falling back to send_task.")
