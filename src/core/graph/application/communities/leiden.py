...
class CommunityDetector:
    ...
    async def detect_communities(
        ...
        # Build a private generation. It becomes visible only after all downstream
        # phases succeed and activate it.
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Failed to discard invalid community generation %s for tenant %s: %s",
                    generation_id,
                    tenant_id,
                    cleanup_error,
                )
        ...
        return {"status": "success", "community_count": count, "generation_id": generation_id}
    ...
    async def _validate_generation(
        ...
        if row.get("community_count", 0) != expected_count or row.get("invalid_links", 0):
            raise RuntimeError(
                f"Community generation validation failed for tenant {tenant_id}: "
                f"expected={expected_count} actual={row.get('community_count', 0)} "
                f"invalid_links={row.get('invalid_links', 0)}"
            )

    async def activate_generation(self, tenant_id: str, generation_id: str) -> None:
        """Publish a validated generation and remove the previous one atomically."""
        result = await self.graph.execute_write(
            """
            MATCH (new:Community)
            ...
            WHERE size(staged) > 0
            FOREACH (community IN staged | SET community.active = true)
            WITH staged
            ...
            FOREACH (old IN old_communities | DETACH DELETE old)
            RETURN size(staged) AS activated
            """,
            {"tenant_id": tenant_id, "generation_id": generation_id},
        )
        activated = result[0].get("activated", 0) if result else 0
        if activated == 0:
