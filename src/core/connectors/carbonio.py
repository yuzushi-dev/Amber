...
<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
  <soap:Header>
    <context xmlns="urn:acmemail">
      <format type="xml"/>
    </context>
  </soap:Header>
  <soap:Body>
    <AuthRequest xmlns="urn:acmemailAccount">
...
class Acme MailConnector(BaseConnector):
    """
    Connector for Acme Acme Mail (Mail, Calendar, Chats).
    Uses a hybrid protocol: XML for Auth, JSON for Data.
    """

    def __init__(self, host: str, email: str = "", password: str = ""):
        self.host = host.rstrip("/")  # e.g., https://mail.acme.com
        self.email = email
    ...
    async def authenticate(self, credentials: dict[str, Any]) -> bool:
        ...
        if not self.email or not self.password:
            logger.error("Acme Mail auth failed: Missing email or password")
            return False
        ...
        headers = {
            ...
            "User-Agent": "AmberAcme MailConnector/1.0"
        }
        ...
        try:
            async with httpx.AsyncClient(verify=False, follow_redirects=True, timeout=30.0) as client:
                ...
                if response.status_code != 200:
                    logger.error(f"Acme Mail auth HTTP error: {response.status_code} - {response.text}")
                    return False
                ...
                if token:
                    ...
                    logger.info(f"Acme Mail authenticated as {self.email}")
                    ...
                    try:
                        info_xml = f"""<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope">
                            ...
                            <soap:Body>
                                <GetInfoRequest xmlns="urn:acmemailAccount"/>
                            </soap:Body>
                        </soap:Envelope>"""
                    ...
                    logger.error("Acme Mail auth failed: No authToken in response")
                    return False

        except Exception as e:
            logger.exception(f"Acme Mail auth exception: {e}")
            return False
    ...
    def _tool_search_chats(self):
        async def search_chats(query: str, limit: int = 10) -> str:
            """Search/list chat conversations in Acme Mail using REST API."""
            if not self.auth_token:
        ...
        async def get_chat_history(person_name: str, limit: int = 20, search_query: str = None) -> str:
            ...
            try:
                async with httpx.AsyncClient(verify=False, timeout=30.0, cookies={"ZM_AUTH_TOKEN": self.auth_token}) as client:
                    ...
                    # Note: Acme Mail updatedAt might be stale for some direct chats, but better than random
                    def parse_room_date(r):
                        ds = r.get("updatedAt", "")
                        if not ds: return ""
                        return ds
                        ...
                        for i in range(0, len(user_ids_list), 5):
                            ...
                            try:
                                ...
                                for u in users_resp.json():
                                    uid = u.get("id")
                                    # Fix: Acme Mail returns 'name', not firstName/lastName
                ...
                try:
                    from src.core.connectors.acme-mail_xmpp import get_chat_history as xmpp_get_history
