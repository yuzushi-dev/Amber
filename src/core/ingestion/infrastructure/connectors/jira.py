...
from src.core.ingestion.infrastructure.connectors.base import BaseConnector
...
class JiraConnector(BaseConnector):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/rest/api/3/myself", auth=(email, api_token)
                )
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    for i in issues:
                        ...
                        results.append(
                            f"- [{key}] {summary} (Status: {status}, Priority: {prio}, Assignee: {assignee_name})"
                        )
    ...
    def _tool_create_issue(self):
        async def create_issue(
            project_key: str, summary: str, description: str, issuetype: str = "Task"
        ) -> str:
