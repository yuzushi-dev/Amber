...
class CarbonioWebSocketXMPP:
    ...
    async def get_xmpp_token(self) -> str | None:
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                if resp.status_code == 200:
                    ...
                    logger.info(
                        f"Got XMPP token: {self.xmpp_token[:30] if self.xmpp_token else 'None'}..."
                    )
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if has_features:
                ...
                logger.warning(
                    "No stream features received, proceeding without auth (likely to fail)"
                )
    ...
    async def _authenticate(self) -> bool:
        ...
        try:
            ...
            if "success" in response.lower():
                ...
                bind_iq = '<iq type="set" id="bind_1"><bind xmlns="urn:ietf:params:xml:ns:xmpp-bind"/></iq>'
                ...
                session_iq = '<iq type="set" id="session_1"><session xmlns="urn:ietf:params:xml:ns:xmpp-session"/></iq>'
    ...
    async def get_room_history(
        self, room_id: str, limit: int = 50, search_query: str = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            ping_stanza = (
                f"<iq type='get' to='carbonio' id='{ping_id}'><ping xmlns='urn:xmpp:ping'/></iq>"
            )
                                ...
                                messages.append(
                                    {
                                        ...
                                        "sender": sender
                                        if sender
                                        else "Participant",  # Placeholder if parsing fails
                                    }
                                )
                        except Exception as e:
            ...
            if search_query:
                ...
                logger.info(
                    f"Filtered to {len(filtered)} messages matching '{search_query}' (content or date)"
                )
                return filtered
