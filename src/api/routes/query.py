        ...
        try:
            ...
            if request.options and request.options.agent_mode:
                ...
                try:
                    ...
                    # Add Connector Tools (e.g. Acme Mail) - DYNAMICALLY LOAD
                    ...
                    async with _async_session_maker() as session:
                        ...
                        result = await session.execute(
                            select(ConnectorState).where(
                                ...
                                ConnectorState.connector_type == "acme-mail"
                            )
                        )
                        c_state = result.scalar_one_or_none()
                        
                        if c_state and c_state.sync_cursor:
                             logger.info("Found Acme Mail state, initializing connector...")
                             # Instantiate and auth
                             ConnectorClass = CONNECTOR_REGISTRY["acme-mail"]
                             ...
                             logger.info("Acme Mail authenticated.")
                            ...
                            logger.info("No active Acme Mail state found.")
