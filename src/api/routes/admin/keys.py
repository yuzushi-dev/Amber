...
async def get_my_key_info(request: Request):
    """
...
@router.delete(
    "/{key_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(verify_admin)]
)
async def revoke_api_key(key_id: str, session: AsyncSession = Depends(get_db_session)):
    """
...
async def link_key_to_tenant(
    key_id: str, link: KeyTenantLink, session: AsyncSession = Depends(get_db_session)
):
...
async def unlink_key_from_tenant(
    key_id: str, tenant_id: str, session: AsyncSession = Depends(get_db_session)
):
