...
async def verify_feedback(feedback_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if query_text:
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Failed to compute embedding for feedback {feedback_id}: {e}"
            )
    ...
    try:
        ...
        await tuning.analyze_feedback_for_tuning(
            ...
            comment=feedback.comment,
            selected_snippets=feedback.metadata_json.get("selected_snippets")
...
async def export_golden_dataset(format: str = "jsonl", db: AsyncSession = Depends(get_db)):
    ...
    query = select(Feedback).where(
        Feedback.tenant_id == tenant_id, Feedback.golden_status == "VERIFIED"
    )
    result = await db.execute(query)
...
async def get_approved_feedback(
    ...
    query = (
        ...
        .where(Feedback.tenant_id == tenant_id, Feedback.golden_status == "VERIFIED")
        .order_by(Feedback.created_at.desc())
        .offset(skip)
        .limit(limit)
    )
...
async def toggle_feedback_active(
    ...
    return ResponseSchema(
        data=True, message=f"Feedback {'activated' if is_active else 'deactivated'}"
    )
