...
from src.api.deps import get_current_tenant_id, get_db_session, verify_group_admin
...
@router.post(
    ...
    dependencies=[Depends(verify_group_admin)],
)
...
@router.patch("/{group_id}", response_model=GroupResponse, dependencies=[Depends(verify_group_admin)])
...
@router.delete("/{group_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(verify_group_admin)])
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_group_admin)],
)
