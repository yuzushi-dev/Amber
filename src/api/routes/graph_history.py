...
class GraphEditHistoryCreate(BaseModel):
    ...
    source_view: Optional[str] = None  # 'global' or 'document:{doc_id}'
...
async def list_history(
    ...
    count_result = await session.execute(text(count_query), {"tenant_id": tenant_id, "status": status} if status else {"tenant_id": tenant_id})
    ...
    return GraphEditHistoryListResponse(
        items=items,
        total=total,
        page=page,
        page_size=page_size
...
async def get_pending_count(
    ...
    result = await session.execute(
        text("SELECT COUNT(*) FROM graph_edit_history WHERE tenant_id = :tenant_id AND status = 'pending'"),
        {"tenant_id": tenant_id}
    )
...
async def apply_pending_edit(
    ...
    if row["status"] != "pending":
        raise HTTPException(status_code=400, detail=f"Edit is not pending (status: {row['status']})")
    ...
    try:
        ...
        await session.execute(
            text("UPDATE graph_edit_history SET status = 'applied', applied_at = :now WHERE id = :id"),
...
async def reject_pending_edit(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="Edit not found")
    if row[0] != "pending":
        raise HTTPException(status_code=400, detail=f"Only pending edits can be rejected (status: {row[0]})")
    
    await session.execute(
        text("UPDATE graph_edit_history SET status = 'rejected' WHERE id = :id"),
        {"id": edit_id}
    )
...
async def undo_applied_edit(
    ...
    if row["status"] != "applied":
        raise HTTPException(status_code=400, detail=f"Only applied edits can be undone (status: {row['status']})")
    ...
    try:
        ...
        if action_type == "connect":
            ...
            if not snapshot:
                raise HTTPException(status_code=400, detail="Merge undo requires snapshot (not available)")
        ...
        # Update status
        await session.execute(
            text("UPDATE graph_edit_history SET status = 'undone' WHERE id = :id"),
            {"id": edit_id}
        )
