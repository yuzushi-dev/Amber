...
class TestStructuredQueryExecutor:
    ...
    async def test_non_structured_returns_none(self, executor):
        ...
        result = await executor.try_execute(
            query="What is machine learning?", tenant_id="tenant_123"
        )
    ...
    async def test_list_documents_execution(self, executor):
        ...
        with (
            patch.object(executor.detector, "detect") as mock_detect,
            patch(
                "src.core.retrieval.application.query.structured_query.get_graph_client"
            ) as mock_get_graph_client,
        ):
            mock_detect.return_value = StructuredIntent(
                query_type=StructuredQueryType.LIST_DOCUMENTS, original_query="list documents"
            )
            ...
            result = await executor.try_execute(query="list documents", tenant_id="tenant_123")
    ...
    async def test_count_documents_execution(self, executor):
        ...
        with (
            ...
            mock_detect.return_value = StructuredIntent(
                query_type=StructuredQueryType.COUNT_DOCUMENTS, original_query="how many documents"
            )
            ...
            result = await executor.try_execute(query="how many documents", tenant_id="tenant_123")
