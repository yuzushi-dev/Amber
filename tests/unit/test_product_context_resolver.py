...
def test_install_resolves_admin():
    ctx = resolve_product_context("How do I install Carbonio?")
    assert ctx.audience == "admin"
...
def test_community_edition_phrase_resolves_ce():
    ctx = resolve_product_context("How do I configure domains in Carbonio Community Edition?")
    assert ctx.edition == "ce"
    assert ctx.audience == "admin"


def test_carbonio_ce_phrase_resolves_ce():
    ctx = resolve_product_context("How to upgrade Carbonio CE on Ubuntu?")
    assert ctx.edition == "ce"
...
def test_ce_user_query():
    ctx = resolve_product_context("How do I use the webmail in Carbonio CE?")
    assert ctx.edition == "ce"
    assert ctx.audience == "user"
...
def test_ambiguous_query_returns_context_object():
    ctx = resolve_product_context("Tell me about Carbonio")
...
def test_explicit_ce_has_higher_confidence_than_unknown():
    ce_ctx = resolve_product_context("Carbonio CE backup configuration")
