...
async def test_streaming():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        data.add_field(
            "file", TEST_FILE_CONTENT, filename=TEST_FILENAME, content_type="application/pdf"
        )
        data.add_field("tenant_id", "default")

        async with session.post(
            f"{API_URL}/documents", data=data, headers={"X-API-Key": API_KEY}
        ) as resp:
            if resp.status != 202:
        ...
        async with session.get(
            f"{API_URL}/documents/{doc_id}/file", headers={"X-API-Key": API_KEY}
        ) as resp:
            if resp.status != 200:
            ...
            async with session.delete(
                f"{API_URL}/documents/{doc_id}", headers={"X-API-Key": API_KEY}
            ) as del_resp:
            ...
            if chunk_count < 2:
                ...
                print(
                    "WARNING: Low chunk count, might not be streaming efficiently or network is too fast."
                )
