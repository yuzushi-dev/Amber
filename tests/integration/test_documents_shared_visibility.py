...
async def _seed_shared_default_document(
    ...
    *,
    ...
    document = Document(
        id=str(uuid.uuid4()),
        tenant_id="default",
        filename="carbonio-admin-guide.md",
        content_hash=str(uuid.uuid4()),
        storage_path="default/carbonio-admin-guide.md",
        status=DocumentStatus.READY,
        source_type="upload",
        metadata_={"content_type": "text/markdown"},
    )
    ...
    if with_chunk:
        session.add(
            Chunk(
                ...
                index=0,
                content="Shared Carbonio admin content",
                tokens=6,
                metadata_={},
...
async def _seed_unshared_default_document(
    ...
    *,
    ...
    document = Document(
        id=str(uuid.uuid4()),
        tenant_id="default",
        filename="carbonio-private-admin-guide.md",
        content_hash=str(uuid.uuid4()),
        storage_path="default/carbonio-private-admin-guide.md",
        status=DocumentStatus.READY,
        source_type="upload",
        metadata_={"content_type": "text/markdown"},
    )
    ...
    if with_chunk:
        session.add(
            Chunk(
                ...
                document_id=document.id,
                index=0,
                content="Private Carbonio admin content",
                tokens=5,
                metadata_={},
                embedding_status=EmbeddingStatus.COMPLETED,
            )
...
async def test_shared_document_chunks_are_visible_to_target_tenant(
    ...
    assert payload[0]["content"] == "Shared Carbonio admin content"
